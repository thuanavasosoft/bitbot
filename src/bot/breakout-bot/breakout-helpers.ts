 ...
 * Adapted for MEXC exchange ICandleInfo type
 */
 ...
 */
export function calculateTrueRange(candles: ICandleInfo[]): number[] {
 ...
 */
export function calculateEMA(candles: ICandleInfo[], period: number): EMAPoint[] {
 ...
 * 
 * @param candles - Array of candle objects with timestamp, openPrice, highPrice, lowPrice, closePrice
 * @param params - Parameters object
 * @returns Object with signal, resistance, support, and other metrics
 */
export function calculateBreakoutSignal(candles: ICandleInfo[], params: SignalParams = {}): SignalResult {
  if (!candles || candles.length === 0) {
    return {
      signal: 'Kangaroo',
      resistance: null,
  ...
  // Volume check (optional confirmation) - MEXC doesn't provide volume in ICandleInfo
  // So we'll skip volume checks for now, setting vol_ok to true
  let vol_ok_up = true;
  let vol_ok_dn = true;
