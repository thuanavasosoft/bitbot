...
class AAWaitForEntryState implements AAState {
  ...
  private async _handlePriceUpdate(price: number) {
    try {
      ...
      console.error("[AAWaitForEntryState] Price listener error:", error);
...
`);
  ...
  async onExit() {
    console.log("Exiting AA Wait For Entry State");
    this.priceListenerRemover && this.priceListenerRemover();
  }
}

export default AAWaitForEntryState;
