...
class AAWaitForResolveState implements AAState {
  ...
  private async _handleExitPriceUpdate(price: number) {
    try {
      ...
      console.error("[AAWaitForResolveState] Price listener error:", error);
  ...
  private async _runTrailingUpdaterLoop() {
    while (!this.trailingUpdaterAbort) {
      try {
        ...
        console.error("[AAWaitForResolveState] Failed to update trailing stop levels:", error);
      }
  ...
  private async _updateTrailingStopLevels() {
    ...
    const candles = await withRetries(
      ...
      {
        label: "[AAWaitForResolveState] getCandles (trailing updater)",
        retries: 5,
        minDelayMs: 5000,
        isTransientError,
  ...
  private async _handleExternalOrderUpdate(update: IWSOrderUpdate) {
    ...
    try {
      ...
      if (!closedPosition) {
        console.warn(`[AAWaitForResolveState] Closed position snapshot missing for id ${activePosition.id}`);
        return;
      }
...
`);
  ...
  async onExit() {
    console.log("Exiting AA Wait For Resolve State");
    this._stopAllWatchers();
  }
}

export default AAWaitForResolveState;
